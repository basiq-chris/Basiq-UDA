...
pub async fn send_request(client: reqwest::Client, request_type: BSAPI::RequestType, method: reqwest::Method, pre_token: Option<Token>, data: Option<String>) -> SXL::Log {
    ...
    match request_type {
        ...
        BSAPI::RequestType::Accounts(user) => {
            match method {
                Method::GET => {
                    let req = client.get(urlbase.to_owned() + format!("/users/{}/accounts", user).as_str())
                    .header(ACCEPT, "application/json")
                    .bearer_auth(token.unwrap());

                    return Log {
                        req: RequestLog::new(&req, vec![Box::new(("user_id".to_string(), user))]),
