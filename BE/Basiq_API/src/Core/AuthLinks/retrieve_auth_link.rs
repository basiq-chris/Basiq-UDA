use SXL::{Log, SXLoggableRequest, AuthLinkRequest, AuthLinkResponse, AuthLink};
use reqwest::header::ACCEPT;



pub fn get(thread_client: reqwest::blocking::Client, token: String, user_id: String) -> Log {
    let req = thread_client.get("https://au-api.basiq.io/users/".to_owned() + &user_id + "/auth_link")
    ...
    let alresp = AuthLinkResponse {
        status: resp.status(),
        headers: resp.headers().clone(),
        data: AuthLink {data: serde_json::from_str(resp.text().unwrap().as_str()).unwrap()},
    };
